namespace Laboratorio_13
{
    partial class Form1
    {
        /// <summary>
        ///  Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        ///  Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        ///  Required method for Designer support - do not modify
        ///  the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            conexion = new Button();
            listBox1 = new ListBox();
            SuspendLayout();
            // 
            // conexion
            // 
            conexion.Location = new Point(224, 27);
            conexion.Margin = new Padding(3, 2, 3, 2);
            conexion.Name = "conexion";
            conexion.Size = new Size(243, 75);
            conexion.TabIndex = 0;
            conexion.Text = "Conectar y desconectar de SQL Server ";
            conexion.UseVisualStyleBackColor = true;
            conexion.Click += conexion_Click;
            // 
            // listBox1
            // 
            listBox1.FormattingEnabled = true;
            listBox1.ItemHeight = 15;
            listBox1.Location = new Point(218, 119);
            listBox1.Name = "listBox1";
            listBox1.Size = new Size(249, 199);
            listBox1.TabIndex = 1;
            listBox1.SelectedIndexChanged += listBox1_SelectedIndexChanged;
            // 
            // Form1
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(700, 338);
            Controls.Add(listBox1);
            Controls.Add(conexion);
            Margin = new Padding(3, 2, 3, 2);
            Name = "Form1";
            Text = "Form1";
            ResumeLayout(false);
        }

        #endregion

        private Button conexion;
        private ListBox listBox1;
    }
}
